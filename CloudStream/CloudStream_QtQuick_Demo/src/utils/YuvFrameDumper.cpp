...
YuvFrameDumper::YuvFrameDumper(int maxFrames)
    : m_enabled(true), m_frameCount(0), m_maxFrames(maxFrames), m_lastWidth(0), m_lastHeight(0), m_yuvFile(nullptr) {
...
void YuvFrameDumper::setMaxFrames(int maxFrames) {
  if (maxFrames <= 0) {
    Logger::warning(QString("[YuvFrameDumper] 无效的最大帧数: %1，保持原值: %2").arg(maxFrames).arg(m_maxFrames));
    return;
  }
...
bool YuvFrameDumper::saveFrame(const TcrI420Buffer& i420Buffer) {
  ...
  if (i420Buffer.width <= 0 || i420Buffer.height <= 0) {
    Logger::error(QString("[YuvFrameDumper] 无效的分辨率: %1x%2").arg(i420Buffer.width).arg(i420Buffer.height));
    return false;
  }

  // 检测分辨率变化
  bool resolutionChanged = (i420Buffer.width != m_lastWidth || i420Buffer.height != m_lastHeight);
  ...
  // 达到最大帧数后自动停止
  if (m_frameCount >= m_maxFrames) {
    Logger::info(QString("[YuvFrameDumper] 已保存 %1 帧，达到最大帧数，停止录制").arg(m_frameCount));
    closeFile();
  }
...
bool YuvFrameDumper::openFile(int width, int height) {
  // 生成文件名
  QString filename = QString("debug_stream_%1x%2.yuv").arg(width).arg(height);
...
bool YuvFrameDumper::writeYuvData(const TcrI420Buffer& i420Buffer) {
  ...
  for (int row = 0; row < i420Buffer.height; ++row) {
    qint64 bytesWritten = m_yuvFile->write(reinterpret_cast<const char*>(i420Buffer.data_y + row * i420Buffer.stride_y),
                                           i420Buffer.width);
  ...
  for (int row = 0; row < uvHeight; ++row) {
    qint64 bytesWritten =
        m_yuvFile->write(reinterpret_cast<const char*>(i420Buffer.data_u + row * i420Buffer.stride_u), uvWidth);
  ...
  for (int row = 0; row < uvHeight; ++row) {
    qint64 bytesWritten =
        m_yuvFile->write(reinterpret_cast<const char*>(i420Buffer.data_v + row * i420Buffer.stride_v), uvWidth);

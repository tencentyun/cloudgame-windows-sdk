 ...
 * 初始化日志系统并启动日志写入线程
 */
Logger::Logger() : m_logLevel(kDEBUG), m_logToFile(false), m_stopThread(false), m_logThread(nullptr) {
  startLogThread();
}
...
void Logger::doRotate() {
  ...
  m_logFilePath = QString("%1/%2_%3.%4").arg(dirPath).arg(baseName).arg(m_currentFileIndex).arg(extension);
}
  ...
  while (true) {
    // 从队列中获取日志消息
    {
      ...
      while (!m_logQueue.empty() && batch.size() < BATCH_SIZE) {
        ...
        // 构造完整的日志消息
        QString logMsg =
            QString("[%1] [%2] [%3] %4").arg(msg.timeStr).arg(msg.threadIdStr).arg(levelStr).arg(msg.message);
...
// 便捷的静态日志记录方法实现
void Logger::debug(const QString& message) { instance()->log(kDEBUG, message); }

void Logger::info(const QString& message) { instance()->log(kINFO, message); }

void Logger::warning(const QString& message) { instance()->log(kWARNING, message); }

void Logger::error(const QString& message) { instance()->log(kERROR, message); }

...
QString CrashDumpHandler::getDumpPath() { return s_dumpPath; }
...
LONG WINAPI CrashDumpHandler::unhandledExceptionFilter(EXCEPTION_POINTERS* exceptionInfo) {
  ...
  if (success) {
    ...
    MessageBoxW(NULL, reinterpret_cast<LPCWSTR>(message.utf16()), L"Crash Detected", MB_OK | MB_ICONERROR);
  } else {
    qCritical() << "Failed to create crash dump!";
    MessageBoxW(NULL, L"Failed to create crash dump file!", L"Crash Handler Error", MB_OK | MB_ICONERROR);
  }
...
bool CrashDumpHandler::createMiniDump(EXCEPTION_POINTERS* exceptionInfo) {
  ...
  QString dumpFileName =
      QString("%1/crash_%2_pid%3_tid%4.dmp").arg(s_dumpPath).arg(timestamp).arg(processId).arg(threadId);

  qCritical() << "Creating dump file:" << dumpFileName;

  // 创建dump文件
  HANDLE hFile = CreateFileW(reinterpret_cast<LPCWSTR>(dumpFileName.utf16()), GENERIC_WRITE,
                             0,  // 不共享
  ...
  MINIDUMP_TYPE dumpType = static_cast<MINIDUMP_TYPE>(MiniDumpWithFullMemory |         // 包含完整内存
  ...
  BOOL result = MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile, dumpType,
                                  exceptionInfo ? &dumpInfo : NULL, NULL, NULL);

...
VideoRenderPaintedItem::VideoRenderPaintedItem(QQuickItem* parent) : QQuickPaintedItem(parent) {
  // 启用鼠标滚轮事件接收
...
bool VideoRenderPaintedItem::hasFrame() const {
  ...
  if (m_frame->frame_type == VideoFrameType::I420_CPU) {
    return m_frame->data_y != nullptr && m_frame->data_u != nullptr && m_frame->data_v != nullptr;
  }
...
void VideoRenderPaintedItem::setStreamingViewModel(QObject* viewModel) {
  ...
  if (!m_streamingViewModel.isNull()) {
    disconnect(this, SIGNAL(touchEventOccurred(int, int, int, int, int, qint64)), m_streamingViewModel.data(),
               SLOT(sendTouchEvent(int, int, int, int, int, qint64)));
    disconnect(this, SIGNAL(mouseWheelOccurred(float)), m_streamingViewModel.data(), SLOT(sendMouseScrollEvent(float)));
  }
  ...
  bool connected = connect(this, &VideoRenderPaintedItem::touchEventOccurred, streamingVM,
                           &StreamingViewModel::sendTouchEvent, Qt::QueuedConnection);
  ...
  bool wheelConnected = connect(this, &VideoRenderPaintedItem::mouseWheelOccurred, streamingVM,
                                &StreamingViewModel::sendMouseScrollEvent, Qt::QueuedConnection);
...
void VideoRenderPaintedItem::handleTouchEvent(int x, int y, int width, int height, int eventType, qint64 timestamp) {
  if (m_streamingViewModel.isNull()) {
  ...
  emit touchEventOccurred(static_cast<int>(transformedPos.x()), static_cast<int>(transformedPos.y()), videoWidth,
                          videoHeight, eventType, timestamp);
}

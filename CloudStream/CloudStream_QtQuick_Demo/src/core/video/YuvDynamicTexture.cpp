...
YuvDynamicTexture::~YuvDynamicTexture() { destroyRhiTexture(); }
...
QSize YuvDynamicTexture::textureSize() const { return m_size; }
...
QRhiTexture* YuvDynamicTexture::rhiTexture() const { return m_rhiTexture; }
...
bool YuvDynamicTexture::createRhiTexture() {
  ...
  // 创建新的RHI纹理对象
  m_rhiTexture = m_rhi->newTexture(QRhiTexture::R8,      // 单通道8位格式（灰度）
                                   m_size,               // 纹理尺寸
                                   1,                    // 采样数（无多重采样）

...
VideoTransformHelper::VideoTransformHelper() : m_rotationAngle(0.0), m_videoWidth(0), m_videoHeight(0) {}
...
QSizeF VideoTransformHelper::calculateRenderSize(const QSizeF& targetSize, const QSizeF& imageSize) const {
  ...
  QSizeF videoSize = (m_videoWidth > 0 && m_videoHeight > 0) ? QSizeF(m_videoWidth, m_videoHeight) : imageSize;
...
QPointF VideoTransformHelper::transformTouchCoordinates(int x, int y, int viewWidth, int viewHeight) const {

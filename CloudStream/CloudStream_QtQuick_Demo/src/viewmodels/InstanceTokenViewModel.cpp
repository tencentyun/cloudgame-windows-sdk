...
InstanceTokenViewModel::InstanceTokenViewModel(ApiService* apiService, QObject* parent)
    : QObject(parent), m_apiService(apiService), m_isBusy(false) {
  // 连接ApiService的成功信号
  connect(m_apiService, &ApiService::androidInstancesAccessTokenCreated, this,
          &InstanceTokenViewModel::onAccessTokenCreated);

  // 连接ApiService的错误信号
  connect(m_apiService, &ApiService::apiError, this, &InstanceTokenViewModel::onApiError);
}

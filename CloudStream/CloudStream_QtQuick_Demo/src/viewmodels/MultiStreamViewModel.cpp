...
MultiStreamViewModel::MultiStreamViewModel(QObject* parent) : QObject(parent) {
...
void MultiStreamViewModel::initialize(const QStringList& instanceIds, const QString& accessInfo, const QString& token) {
...
void MultiStreamViewModel::onVisibilityChanged(const QStringList& visibleIds, const QStringList& invisibleIds) {
  ...
  Logger::info(QString("可见实例 (%1): %2").arg(visibleIds.length()).arg(visibleIds.join(", ")));
  Logger::info(QString("不可见实例 (%1): %2").arg(invisibleIds.length()).arg(invisibleIds.join(", ")));
...
void MultiStreamViewModel::switchStreamingInstances(const QStringList& streamingIds) {
  ...
  tcr_session_switch_streaming_instances(m_session, result.pointers.data(),
...
void MultiStreamViewModel::registerVideoRenderItem(const QString& instanceId, QObject* item) {
  auto vrItem = qobject_cast<VideoRenderItem*>(item);
  if (!vrItem) {
    Logger::warning(QString("[registerVideoRenderItem] 类型转换失败: %1").arg(instanceId));
    return;
  }
...
void MultiStreamViewModel::connectMultipleInstances(const QStringList& allInstanceIds,
                                                    int concurrentStreamingInstances) {
  ...
  Logger::info(QString("[connectMultipleInstances] Session 创建成功，管理 %1 个实例").arg(allInstanceIds.size()));
  ...
  if (!result.pointers.empty()) {
    tcr_session_access_multi_stream(m_session, result.pointers.data(), static_cast<int32_t>(result.pointers.size()));
    Logger::debug(
        QString("[connectMultipleInstances] 开始连接实例（多流模式），传入 %1 个实例").arg(result.pointers.size()));
  }
  ...
  Logger::info(QString("[connectMultipleInstances] 已创建单个Session，管理 %1 个实例").arg(allInstanceIds.size()));
}
...
void MultiStreamViewModel::pauseStreaming(const QStringList& instanceIds) {
  ...
  tcr_session_pause_streaming(m_session, nullptr, result.pointers.data(), static_cast<int32_t>(result.pointers.size()));
...
void MultiStreamViewModel::resumeStreaming(const QStringList& instanceIds) {
  ...
  tcr_session_resume_streaming(m_session, nullptr, result.pointers.data(),
                               static_cast<int32_t>(result.pointers.size()));
...
void MultiStreamViewModel::SessionEventCallback(void* user_data, TcrSessionEvent event, const char* eventData) {
  ...
  switch (event) {
    case TCR_SESSION_EVENT_STATE_CONNECTED: {
      Logger::info(
          QString("[SessionEventCallback] Session 连接成功，管理 %1 个实例").arg(self->m_allInstanceIds.size()));
...
void MultiStreamViewModel::VideoFrameCallback(void* user_data, TcrVideoFrameHandle frame_handle) {
  ...
  if (frame_buffer->type == TCR_VIDEO_BUFFER_TYPE_I420) {
    ...
    frameDataPtr.reset(new VideoFrameData(frame_handle, i420Buffer.data_y, i420Buffer.data_u, i420Buffer.data_v,

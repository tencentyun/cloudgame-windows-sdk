...
当需要同时拉取多个云手机实例的实时画面时，可使用多路子流模式。与群控或单连模式不同，多路子流每个实例产生独立的视频流，可在视频帧回调中通过 `instance_id` 区分。

**核心流程：** 创建会话配置（指定 `concurrentStreamingInstances`） → 创建会话 → 调用 `tcr_session_access_multi_stream` 连接所有实例 → 动态切换拉流实例

```cpp
// 步骤1：创建会话配置
TcrSessionConfig config = tcr_session_config_default();

// 自定义串流参数（建议使用较低的码率和帧率以支持多路）
...
config.concurrentStreamingInstances = 10;      // 最大同时拉流实例数（默认1，最大200）

// 步骤2：创建会话
TcrSessionHandle tcrSession = tcr_client_create_session(tcrClient, &config);

// 步骤3：连接所有实例（每个实例产生独立的视频流）
const char* allInstances[] = {"cai-xxx-001", "cai-xxx-002", "cai-xxx-003", "cai-xxx-004", "cai-xxx-005"};
tcr_session_access_multi_stream(tcrSession, allInstances, 5);
```

##### 动态切换拉流实例

当只需要拉取部分实例的画面时（如列表滚动时仅拉取可见实例），可通过 `tcr_session_switch_streaming_instances` 动态切换。新列表必须是原始实例列表`allInstances`的子集，且数量不能超过 `concurrentStreamingInstances`：

```cpp
// 根据可见区域动态切换：仅拉取可见的 3 个实例
const char* visibleInstances[] = {"cai-xxx-001", "cai-xxx-003", "cai-xxx-005"};
tcr_session_switch_streaming_instances(tcrSession, visibleInstances, 3);
```

##### 视频帧回调中区分实例

多路子流模式下，每个视频帧都携带 `instance_id`，业务可在回调中据此将帧分发到对应的渲染组件：
...
static void VideoFrameCallback(void* user_data, TcrVideoFrameHandle frame_handle) {
    ...
    // 通过 instance_id 区分不同实例的视频流
    const char* instanceId = buffer->instance_id;

    // 根据 instanceId 将视频帧分发到对应的渲染组件
    // 注意：需要调用 tcr_video_frame_add_ref 增加引用计数
    // 使用完毕后调用 tcr_video_frame_release 释放
}
```

> **注意**：
> - `tcr_session_access_multi_stream` 与 `tcr_session_access` 互斥，同一个 Session 只能使用其中一种模式。
> - 多路子流建议使用较低的码率和帧率，以支持更多并发实例。
> - 视频帧回调在非主线程中执行，渲染到 UI 时需要注意线程安全。
...
`TCR_SESSION_EVENT_TOKEN_EXPIRED` 表示当前会话的鉴权 Token 已经到期，业务收到此事件后可断开连接或重新获取 Token 并重新创建会话连接：
...
static void SessionEventCallback(void* user_data, TcrSessionEvent event, const char* eventData) {
    switch (event) {
        ...
        case TCR_SESSION_EVENT_TOKEN_EXPIRED: {
            // eventData 为 JSON 字符串，包含过期实例信息
            // 例如: {"instanceId": "cai-xxxx-xxx"}
            QJsonDocument doc = QJsonDocument::fromJson(QString::fromUtf8(eventData).toUtf8());
            QString instanceId;
            if (doc.isObject()) {

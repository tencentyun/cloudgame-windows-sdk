...
QSGMaterialShader* YuvMaterial::createShader(QSGRendererInterface::RenderMode) const { return new YuvMaterialShader(); }

void YuvMaterial::setYuvData(const uint8_t* DataY, const uint8_t* DataU, const uint8_t* DataV, int StrideY, int StrideU,
                             int StrideV, int width, int height, QQuickWindow* window) {
  if (!DataY || !DataU || !DataV || width <= 0 || height <= 0) return;
...
void YuvMaterial::uploadTextures(const uint8_t* DataY, const uint8_t* DataU, const uint8_t* DataV, int StrideY,
                                 int StrideU, int StrideV, int width, int height, QQuickWindow* window) {
  // 辅助函数：创建并配置纹理
  ...
  // 检查纹理尺寸
  // if (m_textureY) qDebug("m_textureY size: %dx%d", m_textureY->textureSize().width(),
  // m_textureY->textureSize().height()); if (m_textureU) qDebug("m_textureU size: %dx%d",
  // m_textureU->textureSize().width(), m_textureU->textureSize().height()); if (m_textureV) qDebug("m_textureV size:
  // %dx%d", m_textureV->textureSize().width(), m_textureV->textureSize().height());
}

...
void ApiService::sendRequest(const QString& path, const QJsonObject& data, SuccessHandler onSuccess,
                             bool handleHeaders) {
...
// 公共响应处理实现
void ApiService::processCommonResponse(QNetworkReply* reply, const std::function<void(const QJsonObject&)>& onSuccess) {
  ...
  if (root.contains("Error")) {
    ...
    QString logMsg = QString("API error: code=%1, message=%2, requestId=%3").arg(errorCode, errorMsg, requestId);
    Logger::error(logMsg);
...
void ApiService::describeAndroidInstances(int offset, int limit, const QStringList& instanceIds,
...
void ApiService::connectAndroidGroupInstances(const QStringList& instanceIds, const QStringList& clientSessions) {
  QJsonObject data;
...
void ApiService::connectAndroidInstance(const QString& instanceId, const QString& clientSession) {
  QJsonObject data;

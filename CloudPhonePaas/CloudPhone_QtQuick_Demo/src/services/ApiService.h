...
class ApiService : public QObject {
 ...
 public:
   ...
   */
  void describeAndroidInstances(int offset, int limit, const QStringList& instanceIds = QStringList(),
   ...
   */
  void connectAndroidGroupInstances(const QStringList& androidInstanceIds, const QStringList& clientSessions);
   ...
   */
  void connectAndroidInstance(const QString& androidInstanceId, const QString& clientSession);
 ...
 private:
  ...
  void sendRequest(const QString& path, const QJsonObject& data, SuccessHandler onSuccess, bool handleHeaders = false);

  // 公共响应处理
  void processCommonResponse(QNetworkReply* reply, const std::function<void(const QJsonObject&)>& onSuccess);
};

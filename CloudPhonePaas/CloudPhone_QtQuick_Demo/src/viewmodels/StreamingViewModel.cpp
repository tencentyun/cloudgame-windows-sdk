...
StreamingViewModel::StreamingViewModel(QObject* parent)
    : QObject(parent), m_sessionConnected(false), m_currentRotationAngle(0.0) {
...
StreamingViewModel::~StreamingViewModel() {
  ...
  if (m_videoRenderPaintedItem) {
    disconnect(this, &StreamingViewModel::newVideoFrame, m_videoRenderPaintedItem, &VideoRenderPaintedItem::setFrame);
  ...
  if (m_videoRenderItem) {
    disconnect(this, &StreamingViewModel::newVideoFrame, m_videoRenderItem, &VideoRenderItem::setFrame);
    Logger::info(QString("[~StreamingViewModel] 已断开 VideoRenderItem 连接: %1").arg(m_videoRenderItem->objectName()));
  }
...
void StreamingViewModel::setVideoRenderItem(VideoRenderPaintedItem* item) {
  ...
  if (m_videoRenderPaintedItem) {
    disconnect(this, &StreamingViewModel::newVideoFrame, m_videoRenderPaintedItem, &VideoRenderPaintedItem::setFrame);
    Logger::info(
        QString("[setVideoRenderItem] 断开旧 VideoRenderPaintedItem: %1").arg(m_videoRenderPaintedItem->objectName()));
  }
  ...
  if (m_videoRenderPaintedItem) {
    ...
    // 视频帧从解码线程 -> 主线程 -> 渲染线程
    connect(this, &StreamingViewModel::newVideoFrame, m_videoRenderPaintedItem, &VideoRenderPaintedItem::setFrame,
...
void StreamingViewModel::setVideoRenderItem(VideoRenderItem* item) {
  ...
  if (m_videoRenderItem) {
    disconnect(this, &StreamingViewModel::newVideoFrame, m_videoRenderItem, &VideoRenderItem::setFrame);
    Logger::info(QString("[setVideoRenderItem] 断开旧 VideoRenderItem: %1").arg(m_videoRenderItem->objectName()));
  }
  ...
  if (m_videoRenderItem) {
    // 使用 QueuedConnection | UniqueConnection 确保线程安全且防止重复连接
    connect(this, &StreamingViewModel::newVideoFrame, m_videoRenderItem, &VideoRenderItem::setFrame,
            static_cast<Qt::ConnectionType>(Qt::QueuedConnection | Qt::UniqueConnection));

    Logger::info(QString("[setVideoRenderItem] 已连接 VideoRenderItem: %1").arg(m_videoRenderItem->objectName()));
  } else {
...
void StreamingViewModel::setTcrOperator(BatchTaskOperator* tcrOperator) {
  ...
  Logger::info(QString("[setTcrOperator] 设置批量任务操作器: %1").arg(m_tcrOperator ? "有效" : "无效"));
}
...
void StreamingViewModel::connectSession(const QVariantList& instanceIds, bool isGroupControl) {
  ...
  if (m_session) {
    ...
    if (!result.pointers.empty()) {
      tcr_session_access(m_session, result.pointers.data(), static_cast<int32_t>(result.pointers.size()),
                         isGroupControl);
    } else {
...
void StreamingViewModel::updateCheckedInstanceIds(const QVariantList& instanceIds,
  ...
  if (!addedInstanceIds.isEmpty()) {
    ...
    if (!result.pointers.empty()) {
      tcr_session_join_group(m_instance, result.pointers.data(), static_cast<int32_t>(result.pointers.size()));
    }
  }
  ...
  if (!instanceIds.isEmpty()) {
    auto result = VariantListConverter::convert(instanceIds);
    if (!result.pointers.empty()) {
      tcr_session_set_sync_list(m_instance, result.pointers.data(), static_cast<int32_t>(result.pointers.size()));
    }
  }
}
...
void StreamingViewModel::sendTouchEvent(int x, int y, int width, int height, int eventType, qint64 timestamp) {
...
void StreamingViewModel::enableCameraWithDevice(const QString& deviceId) {
  ...
  if (tcr_session_enable_local_camera_with_config(m_session, &videoConfig)) {
    ...
    Logger::error(QString("[enableCameraWithDevice] 摄像头启用失败: deviceId=%1").arg(deviceId));
  }
}
...
void StreamingViewModel::handleScreenConfigChange(const QString& eventData) {
  ...
  Logger::info(
      QString(
          "[handleScreenConfigChange] 屏幕方向变化: width=%1, height=%2, degree=%3, isLandscape=%4, rotationAngle=%5")
          .arg(width)
          .arg(height)
          .arg(degree)
  ...
  if (m_videoRenderPaintedItem) {
    ...
    Logger::info(
        QString("[handleScreenConfigChange] VideoRenderItem 不支持 setRotationAngle, 旋转角度: %1").arg(rotationAngle));
  } else {
...
void StreamingViewModel::createDataChannel() {
  ...
  static TcrDataChannelObserver s_dc_observer = {
      ...
      [](void* user_data, int32_t port, const TcrErrorCode& code, const char* msg) {
        Logger::error(
            QString("[DataChannel] error on port %1, code=%2, msg=%3").arg(port).arg(code).arg(msg ? msg : ""));
      },

      // 【回调3：接收消息】
      [](void* user_data, int32_t port, const uint8_t* data, size_t size) {
        Logger::debug(QString("[DataChannel] recv message on port %1, size=%2").arg(port).arg(size));
        if (data && size > 0) {
          QByteArray rawData(reinterpret_cast<const char*>(data), static_cast<int>(size));
          Logger::debug(QString("[DataChannel] data content: %1").arg(QString::fromUtf8(rawData)));
        }
      }};
...
void StreamingViewModel::sendDataChannelTestMessage() {
  auto data = QByteArray(
                  "{\"action\":0,\"content\":\"{\\\"type\\\":\\\"PullStreamConnected\\\"}\",\"coords\":[],"
                  "\"heightPixels\":0,\"isOpenScreenFollowRotation\":false,\"keyCode\":0,\"pointCount\":0,"
                  "\"properties\":[],\"text\":\"\",\"touchType\":\"eventSdk\",\"widthPixels\":0}")
                  .toStdString();
...
VideoFrameDataPtr StreamingViewModel::createVideoFrameData(TcrVideoFrameHandle frame_handle,
...
VideoFrameDataPtr StreamingViewModel::createI420FrameData(TcrVideoFrameHandle frame_handle,
                                                          const TcrVideoFrameBuffer* frame_buffer) {
  // I420格式：CPU内存中的YUV数据
  const TcrI420Buffer& i420Buffer = frame_buffer->buffer.i420;

  // 使用I420构造函数创建对象
  return VideoFrameDataPtr(new VideoFrameData(frame_handle, i420Buffer.data_y, i420Buffer.data_u, i420Buffer.data_v,
                                              i420Buffer.stride_y, i420Buffer.stride_u, i420Buffer.stride_v,
                                              i420Buffer.width, i420Buffer.height, frame_buffer->timestamp_us));
}
...
QStringList StreamingViewModel::getCameraDeviceList() {
  ...
  for (int32_t i = 0; i < deviceCount; ++i) {
    ...
    if (tcr_session_get_camera_device(m_session, i, &deviceInfo)) {
      ...
      Logger::info(QString("[getCameraDeviceList] 设备 %1: ID=%2, Name=%3").arg(i).arg(deviceId).arg(deviceName));

      // 将设备ID添加到列表（也可以选择添加设备名称）
      deviceList << deviceId;
    } else {
      Logger::error(QString("[getCameraDeviceList] 获取设备 %1 信息失败").arg(i));
    }
  }
...
QStringList StreamingViewModel::getMicrophoneDeviceList() {
  ...
  for (int32_t i = 0; i < deviceCount; ++i) {
    ...
    if (tcr_session_get_microphone_device(m_session, i, &deviceInfo)) {
      QString deviceId = QString::fromUtf8(deviceInfo.device_id);
      QString deviceName = QString::fromUtf8(deviceInfo.device_name);
      Logger::info(QString("[getMicrophoneDeviceList] 设备 %1: ID=%2, Name=%3").arg(i).arg(deviceId).arg(deviceName));
      deviceList << deviceId;
    } else {

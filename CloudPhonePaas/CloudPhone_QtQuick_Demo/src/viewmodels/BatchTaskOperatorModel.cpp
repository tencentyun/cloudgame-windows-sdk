...
BatchTaskOperatorModel::BatchTaskOperatorModel(BatchTaskOperator* batchTaskOperator, QObject* parent)
  ...
  // 连接批量任务完成和失败信号
  connect(m_batchTaskOperator, &BatchTaskOperator::batchTaskCompleted, this,
          &BatchTaskOperatorModel::onBatchTaskCompleted);
  connect(m_batchTaskOperator, &BatchTaskOperator::batchTaskFailed, this, &BatchTaskOperatorModel::onBatchTaskFailed);
}

// 实现处理对话框信号的方法
void BatchTaskOperatorModel::handleDialogSignal(const QString& dialogType, const QStringList& instanceIds,

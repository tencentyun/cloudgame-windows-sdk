...
void StreamingViewModel::sendTouchEvent(int x, int y, int width, int height, int eventType, int64_t timestamp) {
    if (m_session && m_sessionConnected) {
        ...
        if (m_currentRotationAngle != 0.0 && m_currentVideoWidth > 0 && m_currentVideoHeight > 0) {
            m_transformHelper.transformTouchCoordinates(x, y, width, height,
                                                       cloudX, cloudY, cloudW, cloudH);
        }
        tcr_session_touchscreen_touch(m_session, static_cast<float>(cloudX), static_cast<float>(cloudY),
                                      eventType, cloudW, cloudH, timestamp);
    }
}
...
void StreamingViewModel::setRemoteVideoProfile(int fps, int minBitrate, int maxBitrate,
                                                int width, int height) {
    if (!isSessionReady()) {
        Logger::warning("[setRemoteVideoProfile] session not ready");
        return;
    }
    Logger::info("[setRemoteVideoProfile] fps=" + std::to_string(fps) +
...
std::string StreamingViewModel::getInstanceStats() const {
...
void StreamingViewModel::SessionEventCallback(void* user_data, TcrSessionEvent event, const char* eventData) {
    ...
    if (!::PostMessage(self->m_notifyHwnd, WM_TCR_SESSION_EVENT, eventType,
                       reinterpret_cast<LPARAM>(dataCopy))) {
...
void StreamingViewModel::VideoFrameCallback(void* user_data, TcrVideoFrameHandle frame_handle) {
    ...
    if (!buf || buf->type != TCR_VIDEO_BUFFER_TYPE_I420) {
        Logger::warning("[VideoFrameCallback] invalid buf or not I420, buf=" + std::to_string(reinterpret_cast<uintptr_t>(buf))
    ...
    if (cnt == 0) {
        Logger::info("[VideoFrameCallback] first frame received: "
                     + std::to_string(i420.width) + "x" + std::to_string(i420.height)
                     + " notifyHwnd=" + std::to_string(reinterpret_cast<uintptr_t>(self->m_notifyHwnd)));
    }

    // Copy I420 data into owned memory
    auto frameData = std::make_shared<VideoFrameData>(
        frame_handle,
    ...
    if (!::PostMessage(self->m_notifyHwnd, WM_TCR_VIDEO_FRAME, 0,
                       reinterpret_cast<LPARAM>(heapPtr))) {
        Logger::error("[VideoFrameCallback] PostMessage failed (hwnd=" + std::to_string(reinterpret_cast<uintptr_t>(self->m_notifyHwnd)) + ")");
...
void StreamingViewModel::handleVideoFrameMessage(WPARAM /*wParam*/, LPARAM lParam) {
    ...
    if (cnt == 0) {
        Logger::info("[handleVideoFrameMessage] first frame on UI thread: "
                     + std::to_string(frame->width) + "x" + std::to_string(frame->height)
                     + " onNewVideoFrame=" + std::to_string(onNewVideoFrame ? 1 : 0));
    }
...
void StreamingViewModel::handleScreenConfigChange(const std::string& eventData) {
    ...
    Logger::info("[handleScreenConfigChange] width=" + std::to_string(width) +
                 " height=" + std::to_string(height) +
                 " degree=" + degree +
                 " isLandscape=" + std::string(isLandscape ? "true" : "false") +
...
void StreamingViewModel::createDataChannel() {
    static TcrDataChannelObserver s_dc_observer = {
        ...
        [](void* /*user_data*/, int32_t port, const TcrErrorCode& code, const char* msg) {
            Logger::error("[DataChannel] error port=" + std::to_string(port) +
                          " code=" + std::to_string(code) + " msg=" + (msg ? msg : ""));
        },

        // Message received
        [](void* /*user_data*/, int32_t port, const uint8_t* data, size_t size) {
            Logger::debug("[DataChannel] recv port=" + std::to_string(port) +
    ...
    m_dataChannel = tcr_session_create_data_channel(
        m_session, DATA_CHANNEL_PORT, &s_dc_observer, DATA_CHANNEL_TYPE);
...
void StreamingViewModel::sendDataChannelTestMessage() {
    ...
    TcrErrorCode code = tcr_data_channel_send(
        m_dataChannel, reinterpret_cast<const uint8_t*>(data.data()), data.size());
